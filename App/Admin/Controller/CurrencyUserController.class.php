...
class CurrencyUserController extends AdminController {
	...
	public function MemberQianbaoChongzhiUrl(){
		...
		if(!empty($phone)){
			$name=M("Member")->where("phone='{$phone}'")->find();
		...
		//给分页传参数
		setPageParameter($Page, array('cuid'=>I("cuid"),'phone'=>$name['member_id']));
		$show       = $Page->show();// 分页显示输出// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
		$field=C("DB_PREFIX")."currency_user.*,".C("DB_PREFIX")."member.username,".C("DB_PREFIX")."member.member_id,".C("DB_PREFIX")."member.phone,".C("DB_PREFIX")."currency.currency_name";
	...
	public function MemberQianbaoTibiUrl(){
		...
		setPageParameter($Page, array('cuid'=>I("cuid"),'phone'=>$name['member_id']));
		$show       = $Page->show();// 分页显示输出// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
		$field=C("DB_PREFIX")."qianbao_address.*,".C("DB_PREFIX")."member.phone,".C("DB_PREFIX")."currency.currency_name";
